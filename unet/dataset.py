...
def getTrainingTestingData(path, batch_size):
    ...
    training = torch.utils.data.Subset(transformed_training, torch.arange(2000))
    testing = torch.utils.data.Subset(transformed_testing, torch.arange(2000))
    total = len(training)
    ...
    return DataLoader(train_set, batch_size, shuffle=True), DataLoader(
