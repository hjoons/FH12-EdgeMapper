...
def createCustomDataset(file_path):
    data_file = h5py.File(f'{file_path}', 'r')
    X_train = data_file['images']
    y_train = data_file['depths']

    transform = transforms.ToTensor()

    X_train_tensors = []
    for x in X_train:
        X_train_tensors.append(transform(x))
    y_train_tensors = []
    train_max = 0
    for y in y_train:
        y = transform(y)
...
def createCustomLoader(x, y, bs=1):
    dataset = CustomDataset(x, y)
    loader = DataLoader(dataset, batch_size=1, shuffle=True)
    return loader

def createCustomDataLoader(path, batch_size=1, workers=1):
    random.seed(42)
    x, y = createCustomDataset(path)
    loader = createCustomLoader(x, y, bs=batch_size)
    return loader
